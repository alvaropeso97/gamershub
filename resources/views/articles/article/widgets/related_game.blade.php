@php $juego_rel = \App\Http\Controllers\GamesController::devolverJuego($id->juego_rel); @endphp
<div class="widget widget-game" style="background-image: url('{{Config::get('constants.S1_URL')}}/juegos/img/{{$juego_rel->img_box}}');">
    <div class="overlay">
        ...
        @foreach(\App\Http\Controllers\GamesController::devolverPlataformas($juego_rel->id) as $plataforma) <a href="/plataforma/{{$plataforma->alias}}"><span  class="label" style="background:{{$plataforma->color}};">{{$plataforma->nombre}}</span></a> @endforeach
        ...
        <span class="font-size-13">{{\App\Article::devolverFecha($juego_rel->fecha_lanzamiento)}}</span>
