...
@if($id->tipo == "vid")
        ...
        <section class="hero bg-white border-bottom-1 border-grey-200">
            <div class="container" >
                <div class="page-header">
                    @foreach(\App\Http\Controllers\CategoriasController::devolverCategorias($id->id) as $categoria) <a href="/categoria/{{$categoria->alias}}"><span  class="art_categoria categoria_borde" style="border-color:{{$categoria->color}}; color: #000;">{{$categoria->nombre}}</span></a> @endforeach</p>
    ...
    @endforeach
@else
    <section class="hero hero-review height-500" style="background-image: url('{{Config::get('constants.S1_URL')}}/noticias/{{$id->img}}'); height: 400px; ">
        <div class="hero-bg"></div>
        <div class="container">
            <div class="page-header">
                @foreach(\App\Http\Controllers\CategoriasController::devolverCategorias($id->id) as $categoria) <a href="/categoria/{{$categoria->alias}}"><span  class="art_categoria categoria_borde" style="border-color:{{$categoria->color}};">{{$categoria->nombre}}</span></a> @endforeach</p>
