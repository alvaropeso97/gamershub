<div class="widget widget-game" style="background-image: url('{{Config::get('constants.S1_URL')}}/juegos/img/{{$id->img_box}}');">
    <div class="overlay">
        ...
        @foreach(\App\Http\Controllers\JuegosController::devolverPlataformas($id->id) as $plataforma) <a href="/plataforma/{{$plataforma->alias}}"><span  class="label" style="background:{{$plataforma->color}};">{{$plataforma->nombre}}</span></a> @endforeach
        ...
        <span class="font-size-13">{{\App\Http\Controllers\ArticulosController::traducirFecha($id->fecha_lanzamiento)}}</span>
